...
static esp_err_t app_start_core(crt_video_standard_t video_standard)
{
    ...
    /* ── Tile layer as fused base + keyed overlay on top ──────────── */
    {
        ...
        crt_compose_add_layer_fused(&s_compose, crt_tile_layer_fetch, crt_tile_scanline_hook,
                                    &s_tile);
        crt_compose_add_layer(&s_compose, overlay_rect_fetch, &s_overlay_rect, 0);

        crt_register_scanline_hook(crt_compose_scanline_hook, &s_compose);
        ESP_LOGI(TAG, "compose: tile %ux%u visible (%ux%u pitch) + overlay [%u,%u]-[%u,%u]",
                 TILE_VISIBLE_W, TILE_VISIBLE_H, TILE_PITCH_W, TILE_PITCH_H, s_overlay_rect.x0,
                 s_overlay_rect.y0, s_overlay_rect.x1, s_overlay_rect.y1);
    }
