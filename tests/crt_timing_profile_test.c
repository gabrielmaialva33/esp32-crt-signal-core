...
static void test_pal_m_profile_uses_system_m_timing_with_pal_chroma_clock(void)
{
    crt_timing_profile_t profile = {0};

    assert(crt_timing_get_profile(CRT_VIDEO_STANDARD_PAL_M, &profile) == ESP_OK);
    assert(profile.standard == CRT_VIDEO_STANDARD_PAL_M);
    assert(profile.sample_rate_hz == 14302446);
    assert(profile.total_lines == 262);
    assert(profile.active_start_line == 0);
    assert(profile.active_lines == 240);
    assert(profile.vsync_start_line == 243);
    assert(profile.vsync_line_count == 6);
...
static void test_active_line_index_is_timing_owned(void)
{
    ...
    assert(crt_timing_get_profile(CRT_VIDEO_STANDARD_PAL_M, &pal_m) == ESP_OK);
    ...
    assert(crt_timing_get_profile_line_type(&pal_m, 0) == CRT_TIMING_LINE_TYPE_ACTIVE);
    assert(crt_timing_get_profile_line_type(&pal_m, 240) == CRT_TIMING_LINE_TYPE_BLANK);
}
